import { NFTStorage } from '@mehulagg/nft.storage'
...
//       console.log('⁂ Try out `nft put <path to files>` to upload some')
//     } else {
...
export async function putCar(firstPath, opts) {
  ...
  let totalSent = 0
  const root = await client.storeCar(carReader, {
    maxRetries,
    onStoredChunk: (size) => {
      totalSent += size
